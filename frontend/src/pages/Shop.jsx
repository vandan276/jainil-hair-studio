  ...
  return (
    <div className="bg-section-pattern min-h-screen">
      ...
      <div className="text-center mb-16">
        <p className="text-[11px] tracking-[0.4em] uppercase text-jainil-muted mb-4">{t("apothecary")}</p>
        <h1 className="font-serif text-4xl md:text-6xl text-jainil-text">{t("jainilShop")}</h1>
        <p className="text-jainil-muted max-w-md mx-auto mt-6">{t("shopSub")}</p>
      </div>

      {/* TOP-LEVEL GENDER TABS */}
      <div className="flex justify-center gap-8 mb-12 border-b border-jainil-border/30 max-w-sm mx-auto pb-4">
        {["Women", "Men", "Accessories"].map((gender) => (
          <button
            ...
            className={`text-sm uppercase tracking-[0.4em] transition-all duration-300 ${activeGender === gender
                ? "text-jainil-gold font-bold scale-110"
      ...
      <div className="flex flex-col md:flex-row gap-4 md:items-center md:justify-between border-b border-jainil-border pb-6 mb-12">
          ...
          {cats.map((c) => {
            ...
            return (
              <button key={c} onClick={() => setFilter(c)} data-testid={`shop-filter-${c}`}
                className={`text-[11px] uppercase tracking-[0.18em] px-4 py-2 transition-colors ${filter === c ? "bg-jainil-text text-white" : "text-jainil-muted hover:text-jainil-text"}`}>
                {label}
              </button>
            );
          })}
        ...
        <input type="search" placeholder={t("searchProducts")} value={search} onChange={(e) => setSearch(e.target.value)} data-testid="shop-search-input"
          className="border border-jainil-border px-4 py-2 text-sm w-full md:w-64 focus:outline-none focus:border-jainil-text" />
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {filtered.map((p) => (
          <Link to={`/shop/${p.id}`} key={p.id} className="product-card group block bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500" data-testid={`product-${p.id}`}>
                  ...
                  <div className="product-image-wrap aspect-[4/5] bg-jainil-surface overflow-hidden relative rounded-t-3xl">
                    {p.video_url ? (
                      <>
                      ...
                      <div className="w-full h-full flex items-center justify-center text-jainil-muted italic text-xs">{t("noProducts")}</div>
                    )}
                  ...
                  <div className="product-info mt-6 text-center space-y-3 px-2 relative">
                    ...
                    <p className="text-[10px] uppercase tracking-[0.3em] text-jainil-gold font-bold">{p.category}</p>
                    <h3 className="font-serif text-lg text-jainil-text leading-tight group-hover:text-jainil-gold transition-colors duration-300 h-12 overflow-hidden">{p.name}</h3>
                    
                    {/* COLOR SELECTION SWATCHES */}
                    <div className="flex justify-center gap-2 py-1">
                      {["#1a1512", "#4a3728", "#8b6b4e", "#c5a076"].map((color, idx) => (
                        <div key={idx} className="w-3 h-3 rounded-full border border-jainil-border/50 p-0.5 hover:border-jainil-gold transition-colors cursor-pointer">
                          <div className="w-full h-full rounded-full" style={{ backgroundColor: color }} />
                        </div>
                      ))}
                    </div>

                    <p className="text-jainil-text font-medium text-lg">₹{p.price.toLocaleString("en-IN")}</p>
                  </div>
          </Link>
        ))}
      </div>
      {filtered.length === 0 && <p className="text-jainil-muted text-center py-20">{t("noProducts")}</p>}
    </div>
    </div>
  );
}
